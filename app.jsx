...
const { motion, useScroll, useTransform, useSpring, useInView, useMotionValue, useAnimationFrame } = window.Motion;
...
const SerpentSegment = ({ leaderX, leaderY, color, size, current, total }) => {
  const x = useSpring(leaderX, { stiffness: 80 - current * 2, damping: 15 + current, mass: 0.5 });
  const y = useSpring(leaderY, { stiffness: 80 - current * 2, damping: 15 + current, mass: 0.5 });

  const opacity = 0.8 * (1 - current / total);
  const glowOpacity = 0.15 * (1 - current / total);

  return (
    <>
      <motion.circle
        style={{ x, y }}
        r={(size * (1 - current / total)) * 2.5}
        fill={color}
        opacity={glowOpacity}
      />
      <motion.circle
        style={{ x, y }}
        r={size * (1 - current / total)}
        fill={color}
        opacity={opacity}
      />
      {current < total && (
...
const FluidSerpent = ({ color, segments = 20, size = 6 }) => {
...
const CursorFlashlight = () => {
  const { mouseX, mouseY } = useContext(MouseContext);
  const flashlightX = useSpring(mouseX, { stiffness: 100, damping: 25 });
  const flashlightY = useSpring(mouseY, { stiffness: 100, damping: 25 });
    ...
    <motion.div
      ...
      style={{
        background: useTransform(
          ...
          ([x, y]) => `radial-gradient(600px circle at ${x}px ${y}px, rgba(249, 93, 19, 0.15), transparent 80%)`
        )
      }}
    />
  );
...
const FloatingSculpture = () => {
  ...
  useEffect(() => {
    ...
    const pointLight1 = new THREE.PointLight(0xf95d13, 2000);
    pointLight1.position.set(30, 30, 30);
    scene.add(pointLight1);

    const pointLight2 = new THREE.PointLight(0x00ffff, 1000); // Subtle blue rim light for prismatic effect
    pointLight2.position.set(-30, -30, 30);
    scene.add(pointLight2);

    const pointLight3 = new THREE.PointLight(0xffffff, 1500);
...
const HeroSerpentField = () => {
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
      ...
      >
        <FluidSerpent color="#f95d13" segments={30} size={10} />
        <FluidSerpent color="#ffffff" segments={20} size={5} />
        <FluidSerpent color="#f95d13" segments={15} size={3} />
      </svg>

      <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,transparent_20%,rgba(7,7,7,0.3)_55%,rgba(7,7,7,0.85)_100%)]" />
    </div>
  );
};
          ...
          >
            ...
            >
              ...
              <div className="space-y-3 text-sm text-offwhite/75">
                {[
                  { label: "User Flow", value: "Fast and clear" },
                  { label: "Visual Tone", value: "Bold contrast" },
                  { label: "Goal", value: "Memorable first screen" }
                ].map((item, i) => (
                  <div key={i} className={`flex items-center justify-between ${i !== 2 ? "border-b border-white/8 pb-3" : ""}`}>
                    <span className="uppercase tracking-[0.18em] text-graytext text-[11px]">{item.label}</span>
